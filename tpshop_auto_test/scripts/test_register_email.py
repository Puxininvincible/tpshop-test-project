...
from tpshop_auto_test.config import PUBLIC_EMAIL
...
def get_data():
    data_list=[]
    # JSON是列表，直接遍历
    for data in read_json("register_email.json"):
        data_list.append([
            data.get("email"),
            data.get("code"),
            data.get("set_pwd"),
...
class Test_Register(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        #点击邮箱注册
        cls.register.page_click_email_register()
        #初始话数据库工具
        cls.db=TPshopDB()
    @classmethod
    ...
    def test_register(self,email,code,set_pwd,confirm_pwd,reference_tel,expect_result,success):
        try:
            # 执行注册操作
            self.register.page_register(email,code,set_pwd,confirm_pwd,reference_tel)
            # 成功分支处理
            if success:
                ...
                # 点击邮箱注册
                self.register.page_click_email_register()
            # 失败分支处理
            else:
            ...
            if email not in PUBLIC_EMAIL:
                self.db.delete_user_by_email(email)
            else:
                log.info(f"[保留] 公共邮箱{email}，不执行删除")